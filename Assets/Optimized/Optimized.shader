...
Shader "unityCookie/tut/beginner/9 Optimized_1" {
	Properties {
	...
	SubShader {
		...
		struct vertexInput {
			...
			float4 tangent : TANGENT;
				...
				//change floats with half4 and change float3 with fixed3 (optimized)*************
		struct vertexOutput {
		...
		vertexOutput vert(vertexInput v) {
			...
			//These lerps can go on top of "if else" statement
			float atten = lerp(1.0 , 1.0/distance, _WorldSpaceLightos0.w);
			float lightDirection = normalize ( lerp(_WorldSpaceLightPos0.xyz , fragmentToLightSource, _WorldSpaceLightPos0.w) );
